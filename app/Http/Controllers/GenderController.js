'use strict'
const Data = use('App/Model/Gender')  // EDIT
const Antl = use('Antl')
...
class GenderController{
  constructor () {
      this.type = ""  // EDIT
      this.key = "gender"  // EDIT
      this.menu = "pos_gender"  // EDIT
      this.download = "Gender.xlsx"  // EDIT
      this.room = "gender"  // EDIT
    }
  * show (request, response){
      const title = Antl.formatMessage('gender.title')  // EDIT
      const data = yield Data.query().orderBy('id', 'desc').fetch()
      const show = yield response.view('pos/pages/gender', {key : this.key ,room : this.room ,title: title , data: data.toJSON()})  // EDIT
      response.send(show)
  }
