'use strict'
const Data = use('App/Model/PaymentMethod')  // EDIT
const Antl = use('Antl')
...
class PaymentMethodController{
  constructor () {
      this.type = ""  // EDIT
      this.key = "payment-method"  // EDIT
      this.menu = "pos_payment_method"  // EDIT
      this.download = "PaymentMethod.xlsx"  // EDIT
      this.room = "payment-method"  // EDIT
    }
  * show (request, response){
      const title = Antl.formatMessage('payment_method.title')  // EDIT
      const data = yield Data.query().orderBy('id', 'desc').fetch()
      const show = yield response.view('pos/pages/payment_method', {key : this.key ,room : this.room,title: title , data: data.toJSON()})  // EDIT
      response.send(show)
  }
    ...
    if(data){
      ...
      var action = 0
        ...
        if(permission.a || permission.e){
          ...
          result.dates_of_debt = data.dates_of_debt
