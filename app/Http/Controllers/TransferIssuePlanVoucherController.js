...
class TransferIssuePlanVoucherController{
  constructor () {
      this.type = 3  // EDIT Receipt = 1 , Issue = 2 , Transfer = 3
      this.key = "transfer-issue-plan-voucher"  // EDIT
      this.menu = "pos_transfer_issue_plan_voucher"
      this.voucher = 'PLAN_TRANSFER_ISSUE_%'
      this.print = 'PCK%'
    }

    * show (request, response){
        const title = Antl.formatMessage('transfer_issue_plan_voucher.title')  // EDIT
      ...
      const data = JSON.parse(request.input('data'))
        if(data){
          var general = yield General.find(data)
              general = yield General.query().where('pos_general_plan.id',data).select('pos_general_plan.*').first()
...
module.exports = TransferIssuePlanVoucherController
