'use strict'
const Data = use('App/Model/Size')  // EDIT
const Antl = use('Antl')
...
class SizeController{
  constructor () {
      this.type = ""  // EDIT
      this.key = "size"  // EDIT
      this.menu = "pos_size"  // EDIT
      this.download = "Size.xlsx"  // EDIT
      this.room = "size"  // EDIT
    }
  * show (request, response){
      const title = Antl.formatMessage('size.title')  // EDIT
      const data = yield Data.query().orderBy('id', 'desc').fetch()
      const show = yield response.view('pos/pages/size', {key : this.key ,room : this.room ,title: title , data: data.toJSON()})  // EDIT
      response.send(show)
  }
