'use strict'
const Data = use('App/Model/Type')  // EDIT
const Antl = use('Antl')
...
class TypeController{
  constructor () {
      this.type = ""  // EDIT
      this.key = "type"  // EDIT
      this.menu = "pos_type"  // EDIT
      this.download = "Type.xlsx"  // EDIT
      this.barcode = "BARCODE_TYPE"
      this.room = "type"  // EDIT
    }
  * show (request, response){
      const title = Antl.formatMessage('type.title')  // EDIT
      const data = yield Data.query().orderBy('id', 'desc').fetch()
      const show = yield response.view('pos/pages/type', {key : this.key ,room : this.room ,title: title , data: data.toJSON()})  // EDIT
      response.send(show)
  }
