var Ermis = function () {
    ...
    var initStatus = function (flag) {
        ...
        if (flag === 1) {//DEFAULT
            ...
            jQuery("#image").data("kendoUpload").clearAllFiles();
            jQuery('input').not('[type=radio]').val("");
            jQuery('textarea').val("");
            jQuery('.k-select').addClass('disabled');
            jQuery('.multiselect').addClass('disabled');
    ...
    var initImageChange = function (){
      jQuery("#image").kendoUpload({ "multiple": false ,
              select: function (e) {
                   var fileReader = new FileReader();
                   fileReader.onload = function (event) {
                       var mapImage = event.target.result;
                        $('#image-preview').find('img').attr('src', mapImage);
    ...
    var onChange = function () {
        ...
        jQuery.each(data.columns, function (k, col) {
            if (col.key === 'text' || col.key === 'password' || col.key === 'number') {
                ...
                jQuery('input[name="' + col.field + '"]:checked').val(dataItem[col.field]);
            }else if (col.key === 'file') {
               var nameImage =  UrlString(dataItem[col.field]) ;
                jQuery('#image-preview').find('img').attr('src', nameImage );
            }
        });
    };
